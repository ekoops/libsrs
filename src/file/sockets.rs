...
/// Returns sockets belonging to the network namespace of the process associated with `proc_view`,
/// indexed by inode number. `eval_sock_ino` is used to determine if a socket must be added or not,
/// as well as if the collection procedure should terminate early.
fn get_by_inode(
...
impl SocketCollector {
    ...
    /// Returns sockets belonging to the network namespace of the process associated with
    /// `proc_view`, whose inode numbers are in `sock_inos`. Scanning stops early once all requested
    /// inodes have been found.
    fn get_sockets_by_inodes(
        proc_view: &ProcView<'_>,
        sock_inos: &HashSet<u64>,
    ) -> io::Result<InodeToSocketMap> {
        let mut sockets_left = sock_inos.len();
        get_by_inode(proc_view, &mut |ino| {
            if sockets_left == 0 {
    ...
    /// Returns all sockets belonging to the network namespace of the process associated with
    /// `proc_view`.
    fn get_all_sockets(proc_view: &ProcView<'_>) -> io::Result<InodeToSocketMap> {
        get_by_inode(proc_view, &mut |_| Verdict::Parse)
    }
    ...
    pub fn ensure_populated(
        ...
        let netns_sockets = match self.strategy {
            SocketCollectionStrategy::AllProcesses => Self::get_all_sockets(proc_view),
            SocketCollectionStrategy::SingleProcess => {
                let sock_inos: HashSet<_> = sock_inos.collect();
                Self::get_sockets_by_inodes(proc_view, &sock_inos)
            }
